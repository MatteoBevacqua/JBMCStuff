...
for f in $files; do 
	...
	jbmc $file --classpath "/lib/core-models.jar":"$folder": --disable-uncaught-exception-check --stop-on-fail --java-threading --trace --graphml-witness "$folder/${file}.graphml"> $folder/$out
done
